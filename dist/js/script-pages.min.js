...
e.stopPropagation()});$('.menu-item-has-children').on('click',function(){$(this).find('.sub-menu').stop().slideToggle();$(document).click(function(e){e.stopPropagation();if($('.menu-item-has-children').has(e.target).length===0){$('.sub-menu').hide()}})})});$(function(){var $win=$(window);$win.on('scroll',function(){if($win.width()>=769){var $navbar=$('.header__navbar');if($win.scrollTop()>90){$navbar.addClass('fixed')}else{$navbar.removeClass('fixed')}}})});$(document).ready(function(){const path=$(location).attr("href").split("contato/",1).reverse().join('');const file='wp-content/themes/pisosabc/source/Support/Sender.php';$('#form').submit(function(){$(".contact__row__form__wrapper__content__status").removeClass("d-none");$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__row__form__wrapper__content__status').append(data);setTimeout(function(){$(".contact__row__form__wrapper__content__status").addClass("d-none")},3000)},error:function(){$('.contact__row__form__wrapper__content__status').append('Erro')}});return!1})});/*!
 * sweetalert2 v7.0.8
